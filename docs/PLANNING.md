...
v0.1 开发前需先确认 [ARCHITECTURE.md](docs/ARCHITECTURE.md) 中的架构基线，再进入具体代码实现。
...
> 目标：v0.1 先实现“提交即记录”，建立 FluxCore 的核心数据来源；远程 push/webhook 事件作为后续增强。
...
- [ ] 监听事件中的 `README.md` 变更，解析 Front Matter（`status`、`version`、`description`）
